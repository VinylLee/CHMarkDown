...
当前稳定版：**1.1.1**。详细变化见 [更新日志](CHANGELOG.md)。
...
- 每个文档独立保留未保存草稿、撤销历史、光标选区和阅读位置；切换时直接恢复，不弹保存提示
- 关闭文件或重启应用后重新打开，仍可恢复上次阅读位置
...
- 关闭外部文件或应用时检查全部相关未保存修改，可保存、放弃或取消
- 笔记列表宽度和折叠状态持久化
...
- 多文档分别保留运行时草稿、撤销历史、光标选区和编辑/预览阅读位置。
- 切换、新建和打开其他文档不再弹保存确认；关闭外部文件或应用时再统一询问。
- 退出时检查所有未保存文档，支持保存非当前文档；关闭按钮改为圆点。
- 阅读位置按文档保存在本机，重新打开或重启恢复；未保存正文仍不会自动写入磁盘。
...
- `CHMarkDown-1.1.1-portable-x64.exe`：单文件便携版，直接运行。
- `CHMarkDown-1.1.1-win-x64.zip`：下载体积较小，完整解压后运行其中的 `CHMarkDown.exe`。
