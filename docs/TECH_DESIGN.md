...
│   │   ├── useDocumentViewState.ts    # 逐文档阅读位置持久化
...
- 切换、新建或打开其他内容时缓存当前文档运行时状态，不弹出保存确认
- 关闭外部文件或关闭应用前，复用统一的未保存修改检查；退出时检查所有缓存文档
...
- 模式切换继续使用当前阅读锚点；文档切换时分别缓存编辑区与预览区位置以及光标选区
- 每个文档的位置状态写入 `localStorage` 的 `chmarkdown:document-view-states`，按稳定文档 ID 区分，最多保留最近 200 项
- 重新选择、关闭后重新打开或应用重启恢复文档时，分别还原该文档的编辑区/预览区阅读位置与光标选区
...
- 渲染进程串行处理菜单、最近文件、拖放和系统入口；打开成功后直接切换并缓存原文档状态
...
- `session.json` 不保存编辑器草稿、标题、正文、图片内容或阅读位置
- 未保存草稿、撤销历史和保存基线按文档保存在渲染进程运行时内存中；退出后不恢复草稿
- 未保存修改只在关闭外部文件或真正退出应用时确认，普通文档切换不再打断
...
- 设置包含主题、编辑器字体与字号、默认打开模式、自动换行、默认启用同步定位、外部图片资源目录名称和托盘图标开关
...
- 使用 `katex` + `markdown-it-texmath`，定界符为 `dollars + brackets`：行内 `$...$`、`\(...\)`，块级 `$$...$$`、`\[...\]`，支持跨行与 `aligned` 等多行环境
- 渲染结果统一经过 DOMPurify 清理；KaTeX 字体与样式随构建打包

### 19.2 编辑区/预览区精确映射

- 源码映射同时写入 `data-source-line` 与 `data-source-end-line`（1-based inclusive），DOMPurify 白名单同步放行
- 编辑区行位置用隐藏镜像测量：镜像显式 `content-box`，宽度为 `clientWidth - 左右 padding`，并复制字体、行高、字距、tab-size、white-space 等全部影响换行的属性；空行折叠光标矩形为零时用“下一行位置减一个行高”修正
- 预览点击按块内 `clientY` 与块高度比例插值映射源码行，`<pre>`/`<code>` 的 padding 不参与映射；坐标系统一使用 viewport 坐标
- `revealEditorLine` 统一执行顺序：`focus({ preventScroll: true })` → `setSelectionRange` → 下一帧滚动，避免浏览器二次调整滚动位置

### 19.3 双向同步滚动状态机

- 所有程序化 `scrollTop` 统一走 `applyScroll`：目标先 clamp 到合法范围，记录浏览器实际采用值，并带 250ms TTL
- 程序化反馈一次性消费：匹配视为反馈忽略，不匹配视为用户输入并作废旧目标
- 平滑导航抑制（800ms）只作用于预览区自身，编辑区真实输入始终优先
- 顶部/底部双向吸附：编辑区顶部↔预览区顶部、编辑区底部↔预览区底部，非边界才用源码锚点插值
- 真实输入侧优先：编辑区滚动取消待执行的预览→编辑同步帧，反之亦然；监听器重绑、模式切换、卸载时清理待执行帧与程序化目标

### 19.4 编辑增强

- 选区包裹在 `beforeinput` 中拦截单个成对符号（含 `“”` `（）` `《》` 等全角符号），同时覆盖直接按键与输入法提交；包裹操作进入撤销历史
- `Ctrl+G` 通过键盘快捷键映射唤出或关闭文档大纲，与 `Ctrl+F`、`Ctrl+R`、`Ctrl+B` 等互不冲突
- 偏好设置新增 `defaultSyncEnabled`：默认 `true`，旧设置文件缺失时按 `true` 迁移；编辑器启动时的同步开关状态读取该设置，设置修改即时生效

## 20. TXT/JSON 纯文本文件

- 文件对话框、拖放、会话恢复、最近文件与系统打开入口统一接受 `.md`、`.markdown`、`.txt` 和 `.json`
- TXT/JSON 继续复用现有外部文档读写、另存为、未保存修改与会话流程，不增加新的文件权限
- 纯文本预览只做 HTML 文本转义并使用 `<pre>` 保留换行，不交给 markdown-it 解析；Markdown 文件保持原有渲染行为
- 纯文本文件不生成 Markdown 大纲，保存内容始终使用原始 UTF-8 文本

## 21. v1.1.0 大文档响应优化

- 输入事件只同步更新 textarea、撤销历史和脏状态；预览、字数、大纲分别通过合并调度器延后 80/140/180 ms 更新
- 连续输入时最长等待分别为 240/420/540 ms，保证派生界面持续刷新，同时避免每个按键重复解析数千行正文
- 纯编辑模式跳过隐藏预览更新；进入分栏或预览前强制同步最新正文。切换文档、打开大纲时同样立即刷新，避免显示旧数据
- 编辑滚动测量缓存换行起始偏移；镜像布局失效后只重建一次，一次二分定位中不重复刷新镜像与全文行数
- 预览滚动缓存 `data-source-line` 节点和行号索引；`v-html` 更新后主动失效，定位源码行使用缓存和二分查找
- 字体、字号、自动换行、容器尺寸或正文变化时明确失效编辑测量缓存；预览 HTML 变化时明确失效预览缓存
- `npm run measure:editor` 使用固定 2000 行、24 次连续输入的负载，分别测量旧式逐次全量计算和 1.1.0 合并计算，默认执行 7 次并报告中位数

同机测量结果（2026-08-10）：

| 管线 | 7 次耗时（ms） | 中位耗时 | 相对改善 |
|------|----------------|----------|----------|
| 每次输入立即全量计算 | 4094.7、3913.9、3980.3、4490.0、3987.3、4731.3、4119.8 | 4094.7 ms | 基线 |
| v1.1.0 合并派生计算 | 628.6、640.9、671.0、646.2、649.2、718.3、1685.9 | 649.2 ms | 84.1% |

测量覆盖 Markdown 渲染、DOMPurify 清理、字数统计和大纲解析；结果用于验证高频计算合并收益，不替代真实窗口中的交互回归。

## 22. v1.1.1 多文档打开状态

- `NoteEditor` 为每个稳定文档 ID 缓存标题、正文、保存基线、独立撤销历史、光标选区及编辑/预览阅读位置
- 切换文档前捕获当前状态，切换后从缓存恢复；新打开文档没有运行时缓存时，从磁盘/本地笔记内容和持久化位置初始化
- 主页面保存接口按传入文档 ID 定位目标，因此退出时可顺序保存非当前文档；任一保存失败都会取消关闭
- 关闭单个或批量外部文件时只检查目标文件，退出应用时检查所有缓存中的未保存文档
- 阅读位置和选区使用严格校验的版本化 `localStorage` 数据；正文与图片草稿不进入持久化位置记录
- 文档列表圆点继续触发原有关闭外部文件/删除本地笔记语义，不改变最近文件移除按钮
